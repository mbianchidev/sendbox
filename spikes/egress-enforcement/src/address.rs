...
/// `docs/egress-enforcement-spike.md` for this residual-risk note.
...
mod tests {
    ...
    fn classifies_alibaba_metadata_outside_rfc1918_and_link_local() {
        // 100.100.100.200 falls inside RFC 6598 shared/CGNAT space
        // (100.64.0.0/10), which is neither RFC 1918 private space nor
        // link-local, so it would otherwise be misclassified as `Global`
        // without an explicit metadata entry.
        let alibaba = Ipv4Addr::new(100, 100, 100, 200);
        assert_eq!(classify(IpAddr::V4(alibaba)), AddressClass::CloudMetadata);
        // A neighboring address in the same /10 that is *not* the metadata
        // literal must not be swept in as CloudMetadata.
        assert_eq!(
