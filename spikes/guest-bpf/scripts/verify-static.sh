...
if ! readelf --program-headers "$binary" >"$program_headers"; then
    echo "readelf could not inspect program headers" >&2
    exit 1
fi
...
if ! readelf --dynamic "$binary" >"$dynamic_section" 2>&1; then
    echo "readelf could not inspect the dynamic section" >&2
    exit 1
fi
