...
impl ConnectStatus {
    /// Stable, lowercase name for deterministic audit/diagnostic output.
    #[must_use]
    pub fn as_str(self) -> &'static str {
        match self {
            ConnectStatus::Ok => "ok",
            ConnectStatus::PolicyDenied => "policy_denied",
            ConnectStatus::ResolutionFailed => "resolution_failed",
            ConnectStatus::ConnectFailed => "connect_failed",
            ConnectStatus::LimitExceeded => "limit_exceeded",
            ConnectStatus::Malformed => "malformed",
            ConnectStatus::UnsupportedProtocol => "unsupported_protocol",
            ConnectStatus::ExpectedIpMismatch => "expected_ip_mismatch",
            ConnectStatus::Timeout => "timeout",
        }
    }
}
...
pub fn encode_request(request: &ConnectRequest) -> Vec<u8> {
    ...
    match &request.target {
        ConnectTarget::Hostname(host) => {
            ...
            // Callers must supply a hostname within protocol bounds; the
            // decoder rejects anything longer, so clamp defensively here.
            buf.push(u8::try_from(bytes.len().min(MAX_HOSTNAME_LEN)).unwrap_or(u8::MAX));
            buf.extend_from_slice(&bytes[..bytes.len().min(MAX_HOSTNAME_LEN)]);
        }
        ConnectTarget::Ip(ip) => {
            buf.push(2);
...
mod tests {
    ...
    fn status_names_are_stable() {
        assert_eq!(ConnectStatus::Ok.as_str(), "ok");
        assert_eq!(
            ConnectStatus::UnsupportedProtocol.as_str(),
